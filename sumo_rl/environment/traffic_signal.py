...
class TrafficSignal:
    ...
    def set_next_phase(self, new_phase: int):
        ...
        Args:
            new_phase (int): Number between [0 ... num_green_phases] 将要设置的下一个绿灯相位的编号
        """
        ...
        if self.green_phase == new_phase or self.time_since_last_phase_change < self.yellow_time + self.min_green:
            ...
            self.green_phase = new_phase # 更新当前的绿灯相位为新的相位
            self.next_action_time = self.env.sim_step + self.delta_time # 当前环境的模拟步长加上时间间隔
            self.is_yellow = True # 当前处于黄灯状态
            self.time_since_last_phase_change = 0 # 自上次相位更改以来的时间为 0
